...
include_once("function/class.category.php");
...
<?php $notify->Notify();?>
        <!-- Aside Start-->
        <?php $homepage->leftSidebar('category');?>
        <!-- Aside Ends-->
...
<?php $homepage->deletePage(TBL_CATEGORY);?>
<script>
$(document).on('click','#input_category_update',function() {
	...
	if($(this).is(':checked'))
	{
		$.ajax({
	type: "POST",
	url: "getdata.php?index=update_category_show&tbl_name=tbl_category",
	data: info,
	...
	else
	{
		$.ajax({
	type: "POST",
	url: "getdata.php?index=update_category_hide&tbl_name=tbl_category",
	data: info,
	success: function(response){
		window.location.reload();
	}
...
$.ajax({
	type: "POST",
	url: "getdata.php?index=category_brnch_mapping_for_pos",
...
$.ajax({
	type: "POST",
	url: "getdata.php?index=category_brnch_mapping_for_invent",
	data: {'pro_id':pro_id,'branch_id':branch_id},
	success: function(response){
		//window.location.reload();
	}
