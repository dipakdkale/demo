<?php
	...
	*****************************************************************************/
	class database {
		...
		public function updateByTwoIds($table,$DataArray,$updateOnField1,$updateOnFieldValue1,$updateOnField2,$updateOnFieldValue2,$printSQL = false) {
			if(count($DataArray) == 0) {
				die($this->error("UPDATE statement has not been created",__FILE__,__LINE__));
			}
			$sql = "UPDATE ".$table." SET ";
			...
			$sql = substr($sql,0,strlen($sql)-2);
			$sql.= " WHERE `".$updateOnField1."` = '".addslashes($updateOnFieldValue1)."' and `".$updateOnField2."` = '".addslashes($updateOnFieldValue2)."' ";
			if($printSQL == true) {
				echo $this->error($sql,__FILE__,__LINE__);
			} else {
				$this->query($sql,__FILE__,__LINE__);
