...
include_once("function/class.measurement.php");
$mgmt_obj=new Measurement();
...
<head>
    ...
    <body>
		<?php $notify->Notify();?>
        <!-- Aside Start-->
        <?php $homepage->leftSidebar('measurement');?>
        <!-- Aside Ends-->
        ...
        <section class="content">
			...
			switch($index)
			{
				case 'addpage':
				if($submit=="Submit")
				$mgmt_obj->addpage('server');
				else
				$mgmt_obj->addpage('local');				
				break;
				case 'editpage':
				if($submit=="Submit")
				$mgmt_obj->editpage('server',$id);
				else
				$mgmt_obj->editpage('local',$id);	
				break;
				default:
				$mgmt_obj->allpage();
			}
...
<?php $homepage->deletePage(TBL_MEASUREMENT);?>
