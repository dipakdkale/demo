...
include_once("function/class.transfer.php");
...
<script type="text/javascript">
    ...
    <body>
        ...
        <!-- Aside Start-->
        <?php $homepage->leftSidebar('transfer');?>
        <!-- Aside Ends-->
        ...
        <section class="content">
			...
			switch($index)
			{
				...
				$transfer_obj->addpage('server');	
				else
				$transfer_obj->addpage('local');				
				break;
				case 'edit':
				if($submit=="Submit")
				$transfer_obj->editpage('server',$id);
				else
				$transfer_obj->editpage('local',$id);	
				break;
				default:
				$transfer_obj->allpage();
			}
			?>
            </div>
            <!-- Page Content Ends -->
            ...
            <!-- Footer Start -->
            <footer class="footer">
                2018 © DIPOS -Digital inventory Pos & Other Service.
            </footer>
            <!-- Footer Ends -->
...
$homepage->deletePage(TBL_PURCHASE_ORDER);?>
<script>
...
$('#tax_amt_total').val(tax_amt_total);
...
var info= 'comp_id='+comp_id;
$.ajax({
	type: "POST",
	url: "getdata.php?index=get_brand",
	data: info,
	success: function(response){
...
var info= 'branch_id='+branch_id;
$.ajax({
	type: "POST",
	url: "getdata.php?index=get_branch",
	data: info,
	success: function(response){
...
var branch_id=$(this).val();							  
var info= 'department_id='+department_id;
$.ajax({
	type: "POST",
	url: "getdata.php?index=get_department",
	data: info,
	success: function(response){
