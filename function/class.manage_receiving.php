...
class ManageReceiving{
    ...
    $sql="select max(id) as total from ".TBL_RECEIVING_ORDER." " ;
    $result= $this->db->query($sql,__FILE__,__LINE__);
    $row = $this->db->fetch_array($result);
    return $row['id']+1;
    }

    function getReceivingId(){
    $sql="select max(id) as total from ".TBL_RECEIVING_ORDER." " ;
    $result= $this->db->query($sql,__FILE__,__LINE__);
    $row = $this->db->fetch_array($result);
    return $row['id']+1;
    ...
    $sql="select stock  from ".TBL_STOCK." where  branch_id='".$id."' and item_id='".$item_id."' " ;
    $result= $this->db->query($sql,__FILE__,__LINE__);
    $row = $this->db->fetch_array($result);
    return $row['stock'];
    }
...
<div class="row">
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title">All Receiving Receipts </h3>
                 <a href="manage_receiving.php?index=addpage" class="btn btn-success m-b-5 pull-right" style="margin-top:-25px;">Add  Receiving Invoice</a>
            </div>
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                            <?php 
                            $sql="select * from ".TBL_RECEIVING_ORDER." where 1 and branch_id='".$_SESSION['branch_id']."' order by id desc" ;
                            $result= $this->db->query($sql,__FILE__,__LINE__);
                            $x=1;
 ...
 <tr class="tbl_row">
                                    <td><?php echo $x;?></td>
                                    <td><?php echo $this->getMoreDetail(TBL_RECEIVING_ORDER,$row['purchase_invoice_no']);?></td>
                                      <td><?php echo  $row['transfer_invoice_no'];?></td>
                                    <td><?php echo $this->getMoreDetail(TBL_RECEIVING_ORDER,$row['receiving_id']);?></td>
                                    <td><?php echo $this->getMoreDetail(TBL_BRANCH,$row['branch_id']);?></td>
                                     <td><?php echo $this->getDepartmentDetails(TBL_RECEIVING_ORDER,$row['department_id']);?></td>
                                    ...
                                    <td>
             <a href="receiving_invoice.php?index=editpage&id=<?php echo $row['id'];?>" title="Edit" rel="tooltip" data-placement="top"><i class="fa fa-edit"></i></a>  
...
<div class="wraper container-fluid">
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="panel panel-default">
                <div class="panel-heading"><h3 class="panel-title">Manage Receiving</h3></div>
                <div class="panel-body">
                    ...
                    <div class="col-md-3">
                       ...
                       <select class="form-control" name="supplier_id" id="supplier_id">
                                ...
                                <?php 
                            $sql="select * from ".tbl_supplier." where 1 and company_id='".$_SESSION['company_id']."'  and supplier_type='External'  order by id desc" ;
                            $result= $this->db->query($sql,__FILE__,__LINE__);
                            $x=1;
                            while($row = $this->db->fetch_array($result))
                            {
                            ?>
                            <option value="<?php echo $row['supplier_id'];?>" <?php if($_POST['supplier_id']==$row['supplier_id']) echo "selected=selected";?>><?php echo $row['name'];?></option>
                            <?php } ?>
                            </select>
                    ...
                    <div class="col-md-3">
                    <div class="form-group">
                            <label for="purchase_invoice_no">PO Number</label>
                           <select class="form-control" id="purchase_invoice_no" name="purchase_invoice_no">
                            ...
                            {
                            ?>
                            <option value="<?php echo $row['id'];?>" <?php if($_POST['id']==$row['id']) echo "selected=selected";?>><?php echo $row['purchase_invoice_no'];?></option>
                            <?php } ?>
                           </select>
                        </div>
                    </div>
            ...
            <!--<div class="col-md-4">
                  ...
                  -->
                    ...
                    <div class="panel-body">
                <div class="row">
                    <div class="col-md-10 col-sm-10 col-xs-10" style="overflow:auto;">
                        <table class="table table-bordered" id="data_table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:10%">Unit Of measument</th>
            ...
            case 'server':
                            ...
                            $this->db->insert(TBL_RECEIVING_ORDER,$insert_sql_array);
                            $last_id=$this->db->last_insert_id();
                            ?>
                            <?php
                            for($i=0;$i<$num_of_items;$i++){
                            $insert_sql_array = array();
                            $insert_sql_array['receiving_id']   = $last_id+1;
                            ...
                            $this->db->insert(TBL_RECEIVING_ITEM,$insert_sql_array);
                            $insert_sql_array = array();
                            $insert_sql_array['item_id']    = $item_id[$i];
                            $insert_sql_array['supplier_id']    = $this->supplier_id;
                            $insert_sql_array['branch_id']  = $_SESSION['branch_id'];
                            ...
                            if($rows==0)
                            { 
                            //echo $unit_of_stock[$i]; exit();
                                //echo $this->getConversion($unit_name[$i],$unit_ofcurrent_stock[i])*$received_quantity[$i];exit();
                                $insert_sql_array = array();
                                $insert_sql_array['item_id']    = $item_id[$i];
                                $insert_sql_array['branch_id']  = $_SESSION['branch_id'];
                            ...
                            $this->db->insert(TBL_STOCK,$insert_sql_array);
                                $sql="select stock from ".TBL_STOCK." where 1 and (branch_id='".$_SESSION['branch_id']."' and item_id='".$item_id[$i]."') " ; 
                            $result= $this->db->query($sql,__FILE__,__LINE__);
                            $rows   =   $this->db->num_rows($result);
                            }
                            else
                            ...
                            //  echo $this->getConversion($unit_name[$i],$unit_ofcurrent_stock[i])*$received_quantity[$i];exit();
                                ...
                                $this->db->updateByTwoIds(TBL_STOCK,$update_sql_array,'branch_id',$_SESSION['branch_id'],'item_id', $item_id[$i]);
                            ...
                            $_SESSION['msg'] = 'Receiving has been added successfully';
                            ?>
                            <script type="text/javascript">
                            window.location = "manage_receiving.php?index=all";
                            </script>
                            <?php
                            exit();
                            } else {
