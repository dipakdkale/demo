...
class PurchaseInvoice{
	...
	function getDepartmentDetails($id,$colname){
	$sql="select * from ".tbl_department." where 1 and id='".$id."' order by id asc" ;
	$result= $this->db->query($sql,__FILE__,__LINE__);
	$row = $this->db->fetch_array($result);
	return $row['$id'];
	}
	...
	$sql="select id  from ".TBL_ITEMS." where name='".$name."' " ;
	$result= $this->db->query($sql,__FILE__,__LINE__);
	$row = $this->db->fetch_array($result);
	return $row['id'];
	}
	...
	function getItemDetailByName($name,$colname){
	...
	$sql="select stock  from ".TBL_STOCK." where item_id='".$id."' " ;
	$result= $this->db->query($sql,__FILE__,__LINE__);
	$row = $this->db->fetch_array($result);
	return $row['stock'];
	}
    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title">All Requisition Invoices</h3>
                 <a href="purchase_invoice.php?index=addpage" class="btn btn-success m-b-5 pull-right" style="margin-top:-25px;">Add Requisition Invoice</a>
            </div>
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12">
                        <table class="table table-bordered">
                            ...
                            <tbody>
							<?php 
                            $sql="select * from ".TBL_PURCHASE_ORDER." where 1 and branch_id='".$_SESSION['branch_id']."' and status!='completed' order by id desc" ;
                            $result= $this->db->query($sql,__FILE__,__LINE__);
                            $x=1;
                            ...
                            ?>
                                <tr class="tbl_row">
                                    ...
                                    <td><?php echo $this->getMoreDetail(TBL_PURCHASE_ORDER,$row['department_id']);?></td>
                                    ...
                                    <td><a href="purchase_invoice.php?index=editpage&id=<?php echo $row['id'];?>" title="Edit" rel="tooltip" data-placement="top"><i class="fa fa-edit"></i></a>|
                                    <?php if($row['status']=='Save') ?>
               <a href="javascript:void(0);"  title="Resume this purchase Invoice." rel="tooltip" data-placement="top"><i class="fa fa-play"></i></a>  
...
<div class="wraper container-fluid">
    <div class="row">
        ...
        <div class="col-md-120">
            <div class="panel panel-default">
                <div class="panel-heading"><h3 class="panel-title">Add Requisition Invoice</h3></div>
                <div class="panel-body">
                    <form role="form" method="post" action="">
                    <div class="col-md-3">
                    <div class="form-group">
                            <label for="fname"> Requisition Invoice For</label><br />

                            <?php echo $this->getMoreDetail(TBL_BRANCH,$_SESSION['branch_id']);?>
                    ...
                    <div class="col-md-3">
                       ...
                       <select class="form-control" name="department_id" id="department_id">
                        ...
                        <?php 
                            ...
                            {
                            ?>
                            <option value="<?php echo $row['id'];?>" <?php if($row['id']==$row['id']) echo "selected=selected";?>><?php echo $row['heading'];?></option>
                            <?php } ?>
                            </select>
                            </div>
                    ...
                    <div class="col-md-3">
                        	<div class="form-group">
                        	<label for="projected_sales">Projected Sale (%)</label>
                            <input type="text" class="form-control" value="100" name="projected_sales" id="projected_sales"  />
                            </div>
                        </div>
                    <div class="col-md-3">
                    <div class="form-group">
                            <label for="user_name">User Name</label>
                            <input type="text" class="form-control" name="user_name" readonly="readonly" value="<?php echo $_SESSION['user_name'];?>" />
                        </div>
                    </div>
                    <div class="col-md-3">
                    <div class="form-group">
                            <label for="user_name">Current Date & Time</label>
                            <input type="text" class="form-control" name="current_date" readonly="readonly" value="<?php echo date('d-m-y h:i:s',time());?>" />
                        </div>
                    </div>
                    <div class="col-md-3">
                    <div class="form-group">
                            <label for="user_name">Order For Date</label>
                           <input type="text" class="form-control" name="manufacturing_date" placeholder="mm/dd/yyyy" id="datepicker">
                        </div>
                    </div>
                    <div class="col-md-3">
                    <div class="form-group">
                            <label for="user_name">Time</label>
                            <div class="bootstrap-timepicker"><input id="timepicker"  name="purchase_time" type="text" class="form-control"/></div>
                        </div>
                    <div class="col-md-12">
                    <div class="form-group">
                            <label for="brand_id">Requisition Number</label>
                            <input type="text" class="form-control" readonly="readonly" id="purchase_invoice_number" name="purchase_invoice_no" value="<?php echo date('dmy',time())."/".$this->getPurchaeseId();?>" />
                        </div>
                    </div>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                     <button type="button" class="btn btn-icon btn-purple m-b-5 add-row"> <i class="fa fa-paper-plane-o"></i>Add Item</button>
                        ...
                        <table class="table table-bordered" id="data_table">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width:10%">Crrent In Hand stock</th>
                                    <th style="width:5%">Tax 1</th>
                                    <th style="width:5%">Tax 2</th>
                                    <th style="width:5%">Tax 3</th>
                                    <th style="width:10%">Average Quantity</th>
                                    <th style="width:10%">Projected Quantity</th>
							...
							<tr>
                                    ...
                                    <th style=""><input type="text" value="0" readonly="readonly" id="tax_amt_total" style="width:80px;" name="tax_amt_total" /></th>
                                    <th style=""><input type="text" value="0" readonly="readonly" id="total_amt" style="width:80px;" name="total_amt" /></th>
                                </tr>
                                <tr class="tr_discount">
                                    ...
                                    <td style=""><input type="text" id="input_discount" style="width:80px;" placeholder="Enter.." name="discount" /></td>
                                </tr>
                                <tr class="tr_discount">
                                	<td style="width:40px;"></td>
                                    ...
                                    <td style=""></td>
                                    <td style=""></td>
                                    <th style="">Bill After Discount</th>
                                    <th style=""><input type="text" id="gross_amt" readonly="readonly" style="width:80px;" name="bill_after_discount" /></th>
                                </tr>
                                <tr>
                                	<td style="width:40px;"></td>
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12">
            			...
            			<button type="button" id="btn_refresh" class=" btn btn-purple">Update Invoice</button>
                   		<button type="button" class="delete-row btn btn-pink">Delete Row</button>
			...
			case 'server':
							...
							$num_of_items=sizeof($item_name); 
							$this->item_name=$item_name;
							$this->item_id=$this->getItemId($item_name);
							$this->unit_of_measurement=$unit_of_measurement;
							$this->in_hand_stock=$in_hand_stock;
							$this->projected_sales=$projected_sales;
							...
							$this->manufacturing_date=date('d-m-y',strtotime($manufacturing_date));
							...
							$insert_sql_array['projected_sales'] 	= $this->projected_sales;
							...
							$insert_sql_array['bill_after_discount'] 	= $this->bill_after_discount;
							$insert_sql_array['grand_total_amt'] 	= $this->grand_total_amt;
							$insert_sql_array['status'] 	= $this->status;
							$this->db->insert(TBL_PURCHASE_ORDER,$insert_sql_array);
							$last_id=$this->db->last_insert_id();
							for($i=0;$i<$num_of_items;$i++){
							$insert_sql_array = array();
							$insert_sql_array['order_id'] 	= $last_id;
							$insert_sql_array['item_id'] 	= $this->getItemId($item_name[$i]);
							$insert_sql_array['item_name'] 	= $item_name[$i];
							$insert_sql_array['hsn_no'] 	= $hsn_no[$i];
							$insert_sql_array['unit_of_measurement'] 	= $measure_unit[$i];
							$insert_sql_array['in_hand_stock'] 	= $in_hand_stock[$i];
							$insert_sql_array['tax1'] 	= $tax1_id[$i];
							$insert_sql_array['tax2'] 	= $tax2_id[$i];
							$insert_sql_array['tax3'] 	= $tax3_id[$i];
							$insert_sql_array['quantity'] 	= $quantity[$i];
							$insert_sql_array['projected_quantity'] 	= $projected_quantity[$i];
							...
							$this->db->insert(TBL_PURCHASE_ITEM,$insert_sql_array);
							}
							for($i=0;$i<$num_of_items;$i++){
							$insert_sql_array = array();
                            $insert_sql_array['item_id']    = $this->getItemId($item_name[$i]);
                            ...
                            $insert_sql_array['hsn_no']     = $hsn_no[$i];
                            $insert_sql_array['unit_of_measurement']    = $measure_unit[$i];
                            $insert_sql_array['in_hand_stock']  = $stock[$i];
                            $insert_sql_array['tax1']   = $tax1_id[$i];
                            $insert_sql_array['tax2']   = $tax2_id[$i];
                            $insert_sql_array['tax3']   = $tax3_id[$i];
                            $insert_sql_array['quantity']   = $quantity[$i];
							...
							window.location = "purchase_invoice.php?index=all";
							</script>
							<?php
							exit();
							} else {
...
echo $JsCodeForFormValidation;
$sql	=	"select * from ".tbl_purchase_order." where id=".$id."";
                ...
                <div class="panel-body">
                    ...
                    <div class="form-group">
                            ...
                            <select class="form-control" name="department_id" id="department_id">
                            	<option value="">Select Department</option>
                                <?php 
                            $sql2="select * from ".TBL_DEPARTMENT." where 1 order by heading asc" ;
                            $result2= $this->db->query($sql2,__FILE__,__LINE__);
                            $x=1;
                            while($row2 = $this->db->fetch_array($result2))
                            {
                            ?>
                            <option value="<?php echo $row2['id'];?>" <?php if($row['department_id']==$row2['id']) echo "selected=selected";?>><?php echo $row2['heading'];?></option>
                            <?php } ?>
                            </select>
...
<?php 
	...
	$update_sql_array['bdepartment_id'] 	= $this->department_id;
